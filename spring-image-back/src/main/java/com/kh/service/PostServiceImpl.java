...
//	private String uploadDir = "C:/Users/user1/Desktop/saveImage";
	...
	public void uploadImages(MultipartFile[] files, String title, String content) {
		...
		try {
			...
			// 이미지들 이미지를 한개 씩 담아오기 map -> 이미지를 한씩 가져올 때 stream을 이용해서 가져오기
			...
			fileNames = List.of(files).stream().map(file -> {
				...
				// file.getOriginalFilename() +로 붙여주면됨
				...
				// 폴더 안에 이미지들 저장하기  완성
				// File.separator = window(\,/) 랑 맥북(/) 모두 경로를 알아서 잡아줌
		...
		post.setImageUrl(String.join(",", fileNames)); // , 찍어서 파일 구분하겠다.
		insertPost(post);
