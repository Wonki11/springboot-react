...
public class PaymentController {
	...
	//payment 라는 주소로 결제 정보가 들어오면 결제확인창구로 넘겨주는 것 결제정보와 결제하고자 하는 사용자의 비밀번호
	...
	private ResponseEntity<?> confirmPayment(Map<String,String> requsetBody, String encodedKey){
		// fetch("https://api.tosspayments.com/v1/payments/confirm"
		String url = "https://api.tosspayments.com/v1/payments/confirm";
		HttpHeaders headers = new HttpHeaders();
		// Authorization : encryptedApiSecretKey
		headers.set("Authorization", encodedKey); // encryptedApiSecretKey 을 위에서 encodedKey 작성함
		headers.set("Content-Type", "application/json");
		
		HttpEntity<Map<String,String>> entity = new HttpEntity<>(requsetBody ,headers);
		
		ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, entity, Map.class);
		return new ResponseEntity<>(response.getBody(), response.getStatusCode());
		
	}
	private ResponseEntity<?> confirmBrandPayment(Map<String,String> requsetBody, String encodedKey){
		// fetch("https://api.tosspayments.com/v1/payments/confirm"
		String url = "https://api.tosspayments.com/v1/brandpay/payments/confirm";
		HttpHeaders headers = new HttpHeaders();
		// Authorization : encryptedApiSecretKey
		headers.set("Authorization", encodedKey); //비밀번호 encryptedApiSecretKey 을 위에서 encodedKey 작성함
		headers.set("Content-Type", "application/json"); // 글자
		
		HttpEntity<Map<String,String>> entity = new HttpEntity<>(requsetBody ,headers);
		
		
		try {
			ResponseEntity<Map> response = restTemplate.exchange(url, HttpMethod.POST, entity, Map.class);// 사용자가 결제할내역 결제명 상점명 
			// restTemplate = HTML
			return new ResponseEntity<>(response.getBody(), response.getStatusCode());
				
		}catch (Exception e) {
			// 사용자 한테 보내는 응답 실패메세지 잘못된 요청으로 안됐어. 라는 상태코드 보낸것
...
/**
 * Entity
 * ResponseEntity : HTTP 요청 또는 응답의 본문(body)와 헤더(headers)를 포함하는 객체
 * HTTP 요청을 보낼 때 본문과 헤더를 설정하고자 할때 사용
 * 본문(body) : 실제 전송될 데이터 ex) 아이디 비밀번호 작성한 글 등
 * 헤더(headers) : HTTP 헤더 정보를 포함 ex) 글자인지 이미지인지 동영상인지 .. 어떤 파일이 들어오는지 누가보내는지 등
 * HttpEntity<문자열,숫자,모르겠으면 비워두기> abc = new HttpEntity<비워져있어도 상관 없음>("요청 본문" , headers);
 * ResponseEntity = 사용자가 요청한 응답을 개발자가 다시 사용자한테 전달할 때 사용 
 * 				   (Response = 응답 / Http를 상속받아서 Http 기능에 응답에 대한 기능을 추가로 설정한 Entity)
 * 				   HttpEntity를 상속받아, Http 응답에 대한 추가적인 정보를 제공 상태코드를 포함하고 있어서
 * 				   클라이언트(사용자)에게 응답을 보낼 때 사용
 * ResponseEntity<String이면 String, Integer면 Integer 여러값이면 ?, 모르겠으면 비워두기 가능> = new ResponseEntity<>("응답본문",headers);
 * 
 * 
 ...
 * RequestEntity<String이면String,Integer면 Integer, 여러값이면 ?, 모르겠으면 비워두기> = new RequestEntity<>("요청본문",headers);
 * RequestEntity<String> req = new RequestEntity<>("요청본문",headers, HttpMethod.Post, url);
 * 
 * 차이점 요약하기 
 * 		클래스			상속관계				주요 사용 목적				 추가 정보 
 * 	HttpEntity		   기본 클래스		    HTTP 요청/응답 본문과 헤더포함		상태코드없음(성공여부)
